...
- Added rc_flpsp = x (Default: Disable 0, Enable: 1) When enabled and upside down in acro or horizon mode only half throttle is applied.
...
-- You can override the preset baronoise value with "bar_noise = X" X = [0(default) - 50cm] Noise is in cm, 0 uses preset sensordefaults (5cm for MS, 12cm for BMP), only change it if you know what u are doing.
